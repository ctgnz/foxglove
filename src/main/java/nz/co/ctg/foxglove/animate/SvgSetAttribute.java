...
public class SvgSetAttribute extends AbstractSvgAnimationElement {
    ...
    /**
     * {@code <set>} has no interpolation at all - it just writes {@code to} at the start of its active duration and
     * either holds it ({@code fill="freeze"}, a JavaFX {@code Animation}'s own default end-of-run behaviour, so
     * nothing extra is needed) or reverts to whatever the target property held before this animation touched it
     * ({@code fill="remove"}, a second {@link Interpolator#DISCRETE} {@link KeyFrame} at the end of {@code dur}
     * writing the pre-animation value captured up front). No value list, {@code calcMode}, or {@code additive}/
     * {@code accumulate} - none of those attributes exist on this element - so this needs none of {@link
     * SvgValueAnimationBuilder}'s machinery.
     */
    @Override
    public Optional<Animation> buildAnimation(Node target, RenderContext context) {
        return SvgAttributeRegistry.resolve(target, getAttributeName()).flatMap(this::buildAnimation);
    }
    ...
    private Optional<Animation> buildAnimation(SvgAttributeBinding<?> rawBinding) {
        ...
        Optional<Object> toValue = binding.parser().apply(to);
        if (toValue.isEmpty()) {
            return Optional.empty();
        }
        Object baseValue = binding.property().getValue();
