...
/**
 * Builds the {@link Animation} for {@code <animateMotion>} via {@code javafx.animation.PathTransition} - close to purpose-built for this, since it already moves a node along a
 * {@link Path} and can orient it to the tangent, matching {@code rotate="auto"}.
 * <p>
 * The motion path comes from one of two sources, in precedence order: a child {@code <mpath>} referencing a {@code
 * <path>} element via {@code xlink:href}, or the inline {@code path} attribute. The third source SMIL allows - a point list via {@code from}/{@code by}/{@code to}/{@code values} -
 * is a documented, deliberate gap: it needs its own value-list resolution (distinct from both #32's scalar/paint values and #34's fixed-arity transform vectors) for a form this
 * issue's own acceptance criteria doesn't test. {@code keyPoints} (progress-along-path per key time) is a further documented gap for the same reason - {@code PathTransition} has
 * no direct support for it and would need a manual {@code Timeline} instead. {@code origin} is defined but has no effect in SVG 1.1, so it is not read at all - a spec no-op, not a
 * gap.
 */
public final class SvgAnimateMotionBuilder {
    ...
    public static Optional<Animation> build(SvgAnimateMotion element, Node target, RenderContext context) {
        ...
        rotateMode.fixedAngle()
            .ifPresent(target::setRotate);
    ...
    /**
     * {@code auto} maps directly onto {@code ORTHOGONAL_TO_TANGENT}. A plain number is a constant tilt applied once, up front - not part of the continuous animation at all, so it
     * is written directly rather than routed through {@code PathTransition}. {@code auto-reverse} (tangent + 180°) is a documented gap: {@code PathTransition} computes its own
     * orientation internally every frame, with no way to layer a further offset on top of it without per-frame interception - it degrades to no rotation, the same as an
     * absent/unparseable {@code rotate}.
     */
