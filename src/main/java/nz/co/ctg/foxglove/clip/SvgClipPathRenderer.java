...
/**
 * Builds the {@link Node} that {@link nz.co.ctg.foxglove.ISvgGraphicsAttributes#applyClip} hands to
 * {@link Node#setClip(Node)}.
 * <p>
 * A plain {@link Group} of the rendered children is enough to union them - verified empirically (a throwaway test,
 * since removed) that a {@code Group} used as a clip covers the union of its children's own coverage, not just the
 * first child or their intersection. This also sidesteps needing every child to be a single {@link javafx.scene.shape.Shape}
 * (as {@code Shape.union} would), since a {@code <clipPath>} may contain a {@code <use>}, which renders to a
 * {@code Group}.
 */
public final class SvgClipPathRenderer {

    /**
     * Renders {@code clipPath}'s content as a clip node, in the coordinate space {@code targetBounds} was measured
     * in (the referencing node's own pre-transform local space - see {@code applyClip}).
     *
     * @param targetBounds the bounding box of the element the clip is being applied to, used both to resolve
     *        {@code clipPathUnits="objectBoundingBox"} and, degenerate (non-positive), to short circuit to an empty
     *        clip rather than build content that would only be discarded
     */
    ...
    private static Node render(SvgClipPath clipPath, RenderContext context, Bounds targetBounds, Set<SvgClipPath> visited) {
        ...
        context.getElementIndex().resolve(clipPath.getClipPath(), SvgClipPath.class)
    ...
    /**
     * Overrides the {@code fill-rule} the child's own normal rendering pipeline already set with {@code clip-rule}
     * instead - the two attributes play the same role (which points a shape's fill contributes across a self
     * -intersecting or {@code fill-rule="evenodd"}-style path), but a shape does not otherwise know when it is being
     * built as clip content rather than painted directly.
     * <p>
     * Applied recursively so a {@code <use>}'s rendered {@link Group} is covered too - which does mean one
     * resolved value is reapplied across that whole subtree, so a deeply nested override (a {@code <use>}
     * referencing content that declares its own, different {@code clip-rule}) is not preserved. Accepted as a known
     * limitation; full fidelity here is materially more machinery for a rare case.
     */
    private static void applyClipRule(ISvgElement child, RenderContext context, Node node) {
        ...
        String clipRule = SvgInheritedStyle.resolve(context, stylable).getClipRule();
