...
/**
 * Substitutes {@code target} with a masked {@link ImageView}, or returns it unchanged when {@code mask} disables
 * rendering entirely.
 * <p>
 * JavaFX has no per-pixel mask analogue to {@code Node.setClip()} - masking here rasterises both {@code target} and
 * {@code mask}'s content to the same pixel rectangle (the resolved mask region) and multiplies {@code target}'s own
 * alpha by the mask's linearRGB luminance, the default SVG masking computes, per pixel. This necessarily flattens
 * the masked subtree to a static raster - nothing inside a mask, or the masked content itself, stays interactive or
 * animatable after this - and always uses linearRGB (this does not read {@code <mask>}'s {@code color-interpolation},
 * which could select sRGB instead).
 */
public final class SvgMaskRenderer {

    /**
     * Supersampling factor applied when rasterising, so a mask still looks crisp when scaled up - at a proportional
     * memory cost. Adjustable at runtime, the same precedent as {@link nz.co.ctg.foxglove.paint.SvgPattern#rasterScale}.
     */
    ...
    public static Node apply(Node target, SvgMask mask, RenderContext context) {
        ...
        replacement.getTransforms().addAll(ownTransforms);
        replacement.getTransforms().add(new Translate(maskRegion.getMinX(), maskRegion.getMinY()));
        ...
        replacement.getTransforms().add(new Scale(1 / rasterScale, 1 / rasterScale));
    ...
    /**
     * The mask region ({@code x}/{@code y}/{@code width}/{@code height}, per {@code maskUnits} - default
     * {@code objectBoundingBox}, unlike {@code clipPathUnits}), in the same pre-transform local frame
     * {@link nz.co.ctg.foxglove.ISvgGraphicsAttributes#applyClip} already resolves {@code objectBoundingBox} against.
     * The SVG-specified defaults ({@code x}/{@code y} = -10%, {@code width}/{@code height} = 120%) apply whenever
     * {@code mask} does not declare its own.
     */
    ...
    /**
     * Renders {@code mask}'s content in a fresh, non-inheriting context - the same reasoning already established for
     * {@code <clipPath>} content in {@link SvgClipPathRenderer} - scaling by {@code targetBounds} when
     * {@code maskContentUnits="objectBoundingBox"} (default {@code userSpaceOnUse}, unlike {@code clipPathUnits}).
     */
    private static Group buildMaskContent(SvgMask mask, RenderContext context, Bounds targetBounds) {
        RenderContext contentContext = RenderContext.root(context.getElementIndex(), context.getViewportWidth(), context.getViewportHeight())
            ...
            .withBaseUri(context.getBaseUri().orElse(null))
            .resolveChild(mask);
        ...
        if (RenderContext.parseUnits(mask.getMaskContentUnits(), UnitsMode.USER_SPACE_ON_USE) == UnitsMode.OBJECT_BOUNDING_BOX
            ...
            content.getTransforms().add(new Translate(targetBounds.getMinX(), targetBounds.getMinY()));
            content.getTransforms().add(new Scale(targetBounds.getWidth(), targetBounds.getHeight()));
        }
        return content;
    }

    /**
     * Snapshots {@code node} at {@code region}, supersampled by {@link #rasterScale}. Wraps in a throwaway
     * {@link Group}/{@link Scene} first, matching {@code SvgPattern.rasterize}'s established precedent - an explicit
     * {@code viewport} is read in the node's own post-transform space, so whatever {@code node}'s own transforms
     * already are, they must map its content into the same frame {@code region} is expressed in (true of {@code
     * target} once its own transforms have been cleared by the caller, and true of mask content by construction -
     * see {@link #buildMaskContent}). The supersampling scale is inserted as the outermost transform (index 0), so
     * it scales that already-correctly-framed result rather than the raw pre-{@code objectBoundingBox}-scale content
     * underneath it.
     */
    private static WritableImage rasterize(Node node, Bounds region) {
        ...
        node.getTransforms().add(0, supersample);
        try {
            return node.snapshot(params, null);
        } finally {
            node.getTransforms().remove(supersample);
        }
    }
    ...
    /**
     * The inverse sRGB electro-optical transfer function, converting one sRGB channel (0-1) to linearRGB - what the
     * SVG masking specification's default luminance-to-alpha computes against, not the sRGB values directly.
     */
