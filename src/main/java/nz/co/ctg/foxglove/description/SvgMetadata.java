...
/**
 * {@code <metadata>}'s content model is "any elements or character data" (#2) - almost always a single {@code <rdf:RDF>} block in practice (RDF/Dublin Core, the schemas the issue
 * itself names), but the specification does not require exactly one child, so this captures every child in document order rather than only the first.
 * <p>
...
public class SvgMetadata extends AbstractSvgElement implements ISvgDescriptiveElement {
    ...
    /**
     * Every child in document order - a {@link String} for character data, an {@link org.w3c.dom.Element} for a child from any namespace (there is no other kind of child this
     * binding has its own class for).
     */
