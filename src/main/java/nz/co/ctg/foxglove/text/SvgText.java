...
@XmlType(name = "text", propOrder = {
    "presentation", "graphics", "text", "conditionalFeatures", "externalResources", "eventListener", "transform", "content"
})
@XmlRootElement(name = "text")
public class SvgText extends AbstractSvgElement implements ISvgTextPositioningElement, ISvgTransformable, ISvgShape<Text> {
    ...
    public Text createShape() {
        graphics.parseStyle(getStyle());
        text.parseStyle(getStyle());
        Text svgText = new Text(x, y, getValue());
        graphics.applyGraphicsProperties(svgText);
        text.applyTextProperties(svgText);
        transform.applyTransforms(svgText);
        return svgText;
    }
