 ...
 * <ul>
 * <li><b>External</b> (#61) - a {@code <font-face>} whose {@code font-family} matches, pointing through {@code <font-face-src><font-face-uri>} at another document holding the
 * {@code <font>} and its glyph outlines. Every one of the 525 documents in the W3C suite labels itself this way.
 * <li><b>Inline</b> (#137) - a {@code <font>} in this very document, identified by its own {@code <font-face>} child. This is how a document that carries its own glyphs declares
 * them, and how 15 of the 17 {@code fonts} chapter tests are written.
 * </ul>
 * A locally declared {@code <font>} is preferred where both would match: it is the more specific declaration, and a document carrying its own glyphs meant to use them.
 * <p>
 * {@code font-family} is a <b>list</b> ({@code "SVGFreeSansASCII,sans-serif"}), so each name is tried in turn and the first that resolves wins. A family naming no SVG font at all
 * - which is almost every real document - resolves to empty and leaves text rendering exactly as it was, through the JavaFX text system.
 * <p>
 * Loaded fonts are cached by resolved URI for the life of the JVM. One 37KB font document is shared by all 525 suite documents, and re-parsing it per document would cost more than
 * everything else the render does.
 */
public final class SvgFontResolver {
     ...
     * <p>
     * The key matters: two documents can each declare a different font under the same family name, and one can declare the same family twice, so anything keyed by name would serve
     * one document's glyphs to another. The element has identity semantics, which is exactly the right key. Weak, so a parsed document that is finished with does not pin its fonts
     * in memory for the life of the JVM.
     */
    private static final Map<SvgFont, SvgFontGlyphs> INLINE_CACHE = Collections.synchronizedMap(new WeakHashMap<>());

    private SvgFontResolver() {
    }

    /**
     * The SVG font {@code fontFamily} names, or null when it names none - the signal to render through the JavaFX text system as before.
     */
    public static SvgFontGlyphs resolve(String fontFamily, RenderContext context) {
        ...
        List<SvgFontFace> faces = context.getElementIndex()
            .getElementsOfType(SvgFontFace.class);
        List<SvgFont> fonts = context.getElementIndex()
     ...
     * <p>
     * This is how an inline font is declared in practice - the {@code <font-face>} sits <i>inside</i> the {@code <font>} and carries no {@code <font-face-src>}, there being
     * nothing to point at. Every one of the 15 {@code fonts}-chapter documents that declares a font of its own is shaped this way.
     * <p>
     * Where two fonts claim the same family the first in document order wins, which is what {@code fonts-desc-01-t} - the one suite document that declares the same family twice -
     * expects.
     */
    ...
    /**
     * Loads the font document {@code href} names, relative to the document's own base URI - the same resolution {@code SvgImage} established for {@code <image>}, and with the same
     * deliberate limitation: a reference that needs fetching from somewhere unrelated to the document is not followed.
     */
    private static SvgFontGlyphs loadFrom(String href, RenderContext context) {
        ...
        if (withoutFragment.isEmpty()) {
            ...
            return context.getElementIndex()
                .resolve(reference, SvgFont.class)
                .map(font -> INLINE_CACHE.computeIfAbsent(font, SvgFontGlyphs::of))
                .filter(SvgFontGlyphs::isUsable)
                .orElse(null);
        }

        URI resolved = context.getBaseUri()
            .map(base -> base.resolve(withoutFragment))
            .orElse(null);
        if (resolved == null) {
            return null;
        }
    ...
    private static SvgFontGlyphs parse(URI uri, String fragmentId) {
        try (InputStream in = uri.toURL()
            ...
            List<SvgFont> fonts = document.getElementIndex()
                .getElementsOfType(SvgFont.class);
