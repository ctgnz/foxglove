...
/**
 * Exercises #2: {@code <metadata>} previously bound as {@code @XmlValue}, which only ever captures character data -
 * a document embedding RDF/Dublin Core (the schemas the issue itself names) parsed to nothing but the whitespace
 * around it, silently discarding the whole subtree. {@code @XmlAnyElement}/{@code @XmlMixed} together now capture
 * every child - an element from any namespace as a real DOM {@link Element}, character data as a {@link String} -
 * in document order.
 * <p>
 * Deliberately raw DOM access only, per the user's own scope decision: a typed RDF/Dublin Core accessor API is a
 * separate follow-up, once there is a concrete consumer to design one around.
 */
public class SvgMetadataBindingTest {
    ...
    public void testMetadataCapturesArbitraryNamespacedContentAsRealDomElements() throws Exception {
        ...
        SvgMetadata metadata = svg.getMetadata().orElseThrow(() -> new AssertionError("no metadata parsed"));
    ...
    /**
     * The user's own explicit acceptance criterion for #2: a document carrying RDF/Dublin Core metadata must
     * survive being written back out, not just parsed - the whole point of capturing it rather than discarding it.
     * A structural check (the RDF content is present, correctly nested and correctly qualified in the marshalled
     * output), not a byte-for-byte comparison against the original bytes - XML re-serialization routinely reformats
     * whitespace and may choose different (but equivalent) namespace prefixes, which this test does not depend on.
     */
    @Test
    public void testMetadataRoundTripsThroughWriteAndReparse() throws Exception {
        ...
        SvgMetadata metadata = reparsed.getMetadata().orElseThrow(() -> new AssertionError("metadata lost on round trip"));
        Element rdf = firstElementOf(metadata.getContent());
        assertThat(rdf.getLocalName(), is("RDF"));
        Element description = firstChildElement(rdf);
        Element title = firstChildElement(description);
