...
/**
 * Exercises #22's acceptance criteria: {@code <switch>} renders only the first direct child whose conditional
 * processing attributes all pass, a standalone conditional element is suppressed by an ordinary container the same
 * way {@code display:none} already is, and neither ever throws.
 */
public class SvgSwitchRenderingTest {
    ...
    public void testSwitchRendersOnlyTheFirstPassingChild() throws Exception {
        ...
        svgSwitch.getContent().add(failing);
        svgSwitch.getContent().add(passing);
        svgSwitch.getContent().add(later);

        Group rendered = render(svgSwitch);
        assertThat(rendered.getChildren(), hasSize(1));
        assertThat(rendered.getChildren().get(0).getId(), is("passing"));
    }
    ...
    public void testSwitchSkipsAChildHiddenByDisplayNone() throws Exception {
        ...
        SvgSwitch svgSwitch = new SvgSwitch();
        svgSwitch.getContent().add(hidden);
        svgSwitch.getContent().add(visible);

        Group rendered = render(svgSwitch);
        assertThat(rendered.getChildren(), hasSize(1));
        assertThat(rendered.getChildren().get(0).getId(), is("visible"));
    }
    ...
    public void testStandaloneElementWithFailingConditionIsSuppressedByItsContainer() throws Exception {
        ...
        group.getContent().add(failing);
        group.getContent().add(passing);

        SvgGraphic svg = new SvgGraphic();
        svg.getContent().add(group);
        Group rendered = svg.createGroup(Locale.forLanguageTag("en-NZ"));
        Group renderedGroup = (Group) rendered.getChildren().get(0);
        assertThat(renderedGroup.getChildren(), hasSize(1));
        assertThat(((Rectangle) renderedGroup.getChildren().get(0)).getId(), is("passing"));
    }

    private static Group render(SvgSwitch svgSwitch) {
        SvgGraphic svg = new SvgGraphic();
        svg.getContent().add(svgSwitch);
