...
/**
 * Exercises #31's acceptance criteria - entirely independent of any animation actually running, per the issue's own
 * acceptance criterion: geometry attributes on the four numeric-property shapes resolve to the right property,
 * paint/colour attributes resolve and parse, and an unmappable attribute name is skipped (returns
...
public class SvgAttributeRegistryTest {
    ...
    public void testRectangleParsesPixelAndPercentValues() {
        ...
        assertThat(binding.parser().apply("42"), is(Optional.of(42.0)));
        assertThat(binding.parser().apply("42px"), is(Optional.of(42.0)));
    }
    ...
    public void testFillResolvesAndParsesAPlainColor() {
        ...
        Paint parsed = binding.parser().apply("red").orElseThrow();
        binding.property().setValue(parsed);
    ...
    public void testStrokeResolvesAndParsesAPlainColor() {
        ...
        Paint parsed = binding.parser().apply("blue").orElseThrow();
        binding.property().setValue(parsed);
    ...
    public void testFillUrlReferenceValueParsesToEmptyButThePropertyStillResolves() {
        ...
        assertThat(binding.parser().apply("url(#grad)").isEmpty(), is(true));
    }
    ...
    public void testOpacityResolvesAndParsesAPercentage() {
        ...
        double parsed = binding.parser().apply("50%").orElseThrow().doubleValue();
        binding.property().setValue(parsed);
    ...
    public void testStrokeWidthResolvesAndParses() {
        ...
        binding.property().setValue(binding.parser().apply("3").orElseThrow());
    ...
    public void testGeometryAttributeOfTheWrongShapeTypeIsUnmappable() {
        assertThat(SvgAttributeRegistry.resolve(new Rectangle(), "cx").isEmpty(), is(true));
    }

    @Test
    public void testPointsOnAPolygonIsUnmappable() {
        assertThat(SvgAttributeRegistry.resolve(new Polygon(), "points").isEmpty(), is(true));
    }

    @Test
    public void testAnyAttributeOnANonShapeNodeIsUnmappable() {
        assertThat(SvgAttributeRegistry.resolve(new Group(), "opacity").isEmpty(), is(true));
    }

    @Test
    public void testUnknownAttributeNameIsUnmappable() {
        assertThat(SvgAttributeRegistry.resolve(new Rectangle(), "not-a-real-attribute").isEmpty(), is(true));
    }

    // --- helpers -----------------------------------------------------------

    private static SvgAttributeBinding<?> resolve(javafx.scene.Node node, String attributeName) {
        return SvgAttributeRegistry.resolve(node, attributeName).orElseThrow();
    }
    ...
    private static void assertResolvesAndWrites(javafx.scene.Node node, String attributeName, java.util.function.DoubleSupplier readBack) {
        ...
        double value = binding.parser().apply("77").orElseThrow().doubleValue();
