...
/**
 * Exercises #30's target resolution: {@code xlink:href} when present and resolvable, otherwise the animation
 * element's own parent in the parsed tree - SMIL's default target rule.
 */
public class SvgAnimationTargetsTest {
    ...
    public void testXlinkHrefResolvesToTheReferencedElement() throws Exception {
        ...
        unrelatedParent.getContent().add(animation);

        SvgGraphic svg = new SvgGraphic();
        svg.getContent().add(target);
        svg.getContent().add(unrelatedParent);
        SvgElementIndex index = svg.getElementIndex();

        assertThat(SvgAnimationTargets.resolve(animation, index).orElseThrow(), is(sameInstance((ISvgElement) target)));
    }
    ...
    public void testAbsentHrefFallsBackToTheParent() throws Exception {
        ...
        SvgAnimateAttribute animation = new SvgAnimateAttribute();
        parent.getContent().add(animation);

        SvgGraphic svg = new SvgGraphic();
        svg.getContent().add(parent);
        SvgElementIndex index = svg.getElementIndex();

        assertThat(SvgAnimationTargets.resolve(animation, index).orElseThrow(), is(sameInstance((ISvgElement) parent)));
    }
    ...
    public void testAnimationDirectlyUnderTheDocumentRootTargetsTheRootItself() throws Exception {
        ...
        SvgAnimateAttribute animation = new SvgAnimateAttribute();
        SvgGraphic svg = new SvgGraphic();
        svg.getContent().add(animation);
        SvgElementIndex index = svg.getElementIndex();

        assertThat(SvgAnimationTargets.resolve(animation, index).orElseThrow(), is(sameInstance((ISvgElement) svg)));
    }
