...
/**
 * Render dispatch used to be a hard coded {@code instanceof} chain duplicated in {@code SvgGraphic} and {@code SvgGroup}, which had already drifted: the root skipped invisible
 * groups and shapes but always drew text, while a group drew everything and merely hid itself. These cover the single dispatch that replaced it.
 */
public class ISvgContainerTest {
    ...
    public void testRendersShapeChildren() throws Exception {
        ...
        assertThat(rendered.getChildren()
            .get(0), is(instanceOf(Rectangle.class)));
        assertThat(rendered.getChildren()
            .get(1), is(instanceOf(Circle.class)));
    }
    ...
    public void testRendersNestedGroups() throws Exception {
        ...
        Group renderedOuter = (Group) svg.createGroup()
            .getChildren()
            .get(0);
        Group renderedInner = (Group) renderedOuter.getChildren()
            .get(0);
        assertThat(renderedInner.getChildren(), hasSize(1));
        assertThat(renderedInner.getChildren()
            .get(0)
            .getId(), is("deep"));
    }
    ...
    /**
     * Elements that exist to be referenced or to describe are excluded by not implementing {@link FxGraphic}, rather than by being missing from a list of types the container knows
     * how to draw.
     */
    @Test
    public void testSkipsNonRenderableChildren() throws Exception {
        ...
        svg.getContent()
            .add(new SvgTitle());
        svg.getContent()
            .add(new SvgDescription());
        svg.getContent()
            .add(new SvgMetadata());
        svg.getContent()
            .add(new SvgAnimateAttribute());
        svg.getContent()
            .add(new SvgLinearGradient());

        assertThat(svg.createGroup()
            .getChildren(), is(empty()));
    }
    ...
    public void testDisplayNoneShapeIsAbsentInsideAGroup() throws Exception {
        ...
        Group rendered = (Group) svg.createGroup()
            .getChildren()
            .get(0);
        assertThat(rendered.getChildren(), hasSize(1));
        assertThat(rendered.getChildren()
    ...
    /**
     * The acceptance criterion for the rework: the two dispatch sites disagreed about this, so the same content has to produce the same outcome whether it sits at the root or
     * inside a group.
     */
    @Test
    public void testDisplayNoneBehavesTheSameAtRootAndNested() throws Exception {
        ...
        Group nestedChildren = (Group) nested.createGroup()
            .getChildren()
            .get(0);
        assertThat(rootChildren.getChildren()
            .size(),
            is(nestedChildren.getChildren()
                .size()));
        assertThat(rootChildren.getChildren()
            .get(0)
            .getId(),
    ...
    /**
     * {@code display="none"} removes the element from the scene graph along with everything inside it, rather than adding a hidden node.
     */
    @Test
