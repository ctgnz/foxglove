...
/**
 * Regression cover for #119: seven presentation attributes on {@code <clipPath>} and {@code <mask>} were bound as plain strings, against getters returning {@code SvgPaint},
 * {@code List<Double>}, {@code Double} and {@code StrokeLineJoin}. Reading any of them threw {@code ClassCastException}.
 * <p>
 * That a {@code <clipPath>} never strokes anything of its own is no protection: these properties are <b>inherited</b> (see {@code SvgInheritedStyle}), so the wrongly-typed value
 * travels down to the element's children and the unchecked cast fails as soon as a child shape has its graphics properties applied.
 * <p>
 * Necessarily driven through {@link FoxgloveParser}. Building the same objects in memory goes through the typed setters and stores the right type regardless of what the binding
 * file says, so an in-memory test passes whether the binding is right or wrong - which is exactly how this survived unnoticed, the same trap as #105.
 */
public class ClipAttributeBindingTest {
    ...
    /**
     * The crash as originally reported: a child shape inheriting the mistyped value from its {@code <clipPath>} parent. Rendering is what triggers it, since that is when graphics
    ...
    private static void assertAttributesAreTyped(ISvgGraphicsAttributes element) {
        assertThat(element.getFill()
            .getPaint(), is(Color.RED));
        assertThat(element.getStroke()
            .getPaint(), is(Color.BLUE));
