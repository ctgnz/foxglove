...
/**
 * Smoke-tests #30's new, additive entry point: {@code createAnimatedGraphic} builds the same node {@code createGraphic} alone would (the node registry is a pure side effect of
 * rendering, not a different render), and the registry it populates along the way actually resolves back to the real rendered shape - end to end, through the real rendering
 * pipeline rather than a manually-built registry like {@code SvgAnimationControllerTest}.
 */
public class AnimatedGraphicTest {
    ...
    public void testCreateAnimatedGraphicReturnsAWorkingNode() throws Exception {
        ...
        assertThat(((Group) result.node()).getChildren()
            .size(), is(1));
    }
